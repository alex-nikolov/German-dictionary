describe 'Highscore' do
  describe '#initialize' do
    it 'formats the time correctly' do
      highscore = German::Highscore.new(100, 'highscores_examples.db', 'James')
      time = Time.now
  ...
  describe '#reset' do
    it 'resets highscores in corresponding table' do
      German::Highscore.reset('highscores_examples.db')
      expect(German::Highscore.highscores_to_s('highscores_examples.db')).to eq ''
    end
  end

  describe '#write_to_table' do
    it 'writes highscores to empty and non-empty table' do
      first_highscore = German::Highscore.new(100, 'highscores_examples.db', 'James')
      first_highscore.write_to_table
      expected_result = "100.0\tJames\t#{first_highscore.time}"
      expect(German::Highscore.highscores_to_s('highscores_examples.db')).to eq expected_result

      second_highscore = German::Highscore.new(80, 'highscores_examples.db', 'Rodrigo')
